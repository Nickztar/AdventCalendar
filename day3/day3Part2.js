...
//Keep track of movement for each wire
let x1 = 0;
let y1 = 0;
let x2 = 0;
let y2 = 0;
...
let distance1 = 0;
let distance2 = 0;
const positions1 = new Set(); //Set is like a array except values cant be the same
const positions2 = new Set();
const distMap1 = new Map(); //Map saves every value with a key identifying each value
const distMap2 = new Map();

//Map each of the wires
wire1.forEach((str) => {
    const move = parseInt(str.slice(1));
    switch (str[0]) {
        case 'R': //Right
            evalPosW1(move, 'R');
            break;
...
//This actually maps the wires
function evalPosW1(dist, dir, key) {
    for (let i = 0; i < dist; i++) {
        ...
        distance1++;
        positions1.add(`${x1},${y1}`);
        distMap1.set(`${x1},${y1}`, `${distance1}`);
    }
}
function evalPosW2(dist, dir, key) {
    for (let i = 0; i < dist; i++) {
        switch (dir) {
            case 'R':
                x2++;
        ...
        distance2++
        positions2.add(`${x2},${y2}`);
        distMap2.set(`${x2},${y2}`, `${distance2}`);
    }
}
//Get the intersections from the sets
const intersections = [...positions1].filter(pos => positions2.has(pos));

//Calculate the Manhattan distance and returns the minimum distance from center
function minDistance(intersects){
    let min = Infinity
    intersects.forEach((pos) => {
        const temp = pos.split(",").map(str => Math.abs(Number(str)));
        const distance = temp[0] + temp[1];
        if (distance < min) min = distance;
    });
    return min;
}

//Calculate the steps to take to get to each of the intersections
//Returns the minimum steps required
function minSteps(intersects, w1Map, w2Map){
    let min = Infinity
    intersects.forEach((pos) => {
        const steps = parseInt(w1Map.get(pos)) + parseInt(w2Map.get(pos));
        if (steps < min) min = steps;
    });
    return min;
}

console.log(`The answer is (1): ${minDistance(intersections)}`);
console.log(`The answer is (2): ${minSteps(intersections, distMap1, distMap2)}`);
console.timeEnd("Runtime")

/* The answer is (1): 1285
The answer is (2): 14228
Runtime: 477.595ms */
